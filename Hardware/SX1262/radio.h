...
typedef	struct Radio_s
{
	...
	void (*HalDeinit)(void);
	void (*HalInit)(SPI_HandleTypeDef* hspi, RadioCallbacks_t* callbacks);
	void (*Reset)(void);
	void (*Wakeup)(void);
	...
	void (*OnDioIrq)(void);			// 放在中断中
	void (*ProcessIrqs)(void);		// 放在while)(1)中
	...
	uint8_t(*SetFSKSyncWord)(uint8_t* syncWord);
